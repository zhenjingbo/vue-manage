<template>
    ...
    <div class="ms-doc">
      ...
      <span>{{$route.params.typeId == 2? '新增' : '修改'}}联系人</span>
    </div>
    <div class="modify2">
  ...
  methods: {
    getPerson(id) {
      ...
      api.GetPersonById(id)
        .then(res => {
          this.personSingle = res.data.data;
          this.loaded = true;
        ...
        if (valid) {
          if (this.$route.params.typeId == 1) { //如果是修改
          	 this.$confirm('确定要提交修改?', '提示', {confirmButtonText: '确定',cancelButtonText: '取消',type: 'warning'})
          	  .then(() => {
          	  	api.SetPerson(this.personSingle)
              .then(res => {
              	this.$message({type: 'success',message: '修改成功!'});
                this.$router.push({ path: '/manage/linkman' });
              })
              .catch(error => {
            ...
            api.NewPerson( this.personSingle)
              .then(res => {
                ...
                this.$router.push({ path: '/manage/linkman' });
//              this.$store.dispatch('getPerson', to.params.id);
